...
class WASWTrainDataset(Dataset):
    def __init__(self, args):
        super(WASWTrainDataset, self).__init__()
        self.args = args
        self.rs_ids = []
        self.hazy_ids = []
    ...
    def _init_clean_ids(self):
        ...
        for n in name_list:
            full = os.path.join(self.args.denoise_dir, n)
            if n.strip() in temp_ids and os.path.isfile(full) and os.path.splitext(n)[1] in exts:
                clean_ids.append(full)
...
class DenoiseTestDataset(Dataset):
    def __init__(self, args):
        ...
        try:
            ...
            print(f"Warning: Degradation initialization failed: {e}")
            print("Warning: Will use default noise generation method")
            self.D = None
    ...
    def _generate_default_noise(self, clean_img, sigma):
        import numpy as np
        if clean_img.max() > 1:
    ...
    def _init_clean_ids(self):
        ...
        for name in name_list:
            full_path = os.path.join(self.args.denoise_path, name)
            if os.path.isfile(full_path) and os.path.splitext(name)[1] in exts:
                valid_paths.append(full_path)
        self.clean_ids += valid_paths
    ...
    def __getitem__(self, clean_id):
        ...
        if self.D is not None and hasattr(self, 'degrade_type'):
            try:
                noisy_img = self.D.single_degrade(clean_img, self.degrade_type)
            except Exception as e:
                print(f"Warning: Failed to generate noise using Degradation class: {e}")
                print("Warning: Will use default noise generation method")
                noisy_img = self._generate_default_noise(clean_img, self.sigma)
        elif self.D is not None:
            try:
                noisy_img = self.D.single_degrade(clean_img, 0)
            except Exception as e:
                print(f"Warning: Failed to generate noise using Degradation class: {e}")
                print("Warning: Will use default noise generation method")
                noisy_img = self._generate_default_noise(clean_img, self.sigma)
...
class DerainDehazeDataset(Dataset):
    def __init__(self, args, task="derain", addnoise=False, sigma=None):
        ...
        if self.addnoise:
            try:
                ...
                print(f"Warning: DerainDehazeDataset Degradation initialization failed: {e}")
                print("Warning: Will use default noise generation method")
                self.D = None
    ...
    def _init_input_ids(self):
        if self.task_idx == 0:
            self.ids = []
            root = self.args.derain_path + 'input/'
            name_list = os.listdir(root)
            exts = {'.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.PNG', '.JPG', '.JPEG', '.BMP', '.TIF', '.TIFF'}
            self.ids += [os.path.join(root, n) for n in name_list if os.path.isfile(os.path.join(root, n)) and os.path.splitext(n)[1] in exts]
        elif self.task_idx == 1:
            self.ids = []
            root = self.args.dehaze_path + 'input/'
            name_list = os.listdir(root)
            exts = {'.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.PNG', '.JPG', '.JPEG', '.BMP', '.TIF', '.TIFF'}
            self.ids += [os.path.join(root, n) for n in name_list if os.path.isfile(os.path.join(root, n)) and os.path.splitext(n)[1] in exts]
        elif self.task_idx == 2:
            self.ids = []
            root = self.args.gopro_path +'input/'
            name_list = os.listdir(root)
            exts = {'.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.PNG', '.JPG', '.JPEG', '.BMP', '.TIF', '.TIFF'}
            self.ids += [os.path.join(root, n) for n in name_list if os.path.isfile(os.path.join(root, n)) and os.path.splitext(n)[1] in exts]
        elif self.task_idx == 3:
            self.ids = []
            root = self.args.enhance_path + 'input/'
            name_list = os.listdir(root)
            exts = {'.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.PNG', '.JPG', '.JPEG', '.BMP', '.TIF', '.TIFF'}
            self.ids += [os.path.join(root, n) for n in name_list if os.path.isfile(os.path.join(root, n)) and os.path.splitext(n)[1] in exts]
    ...
    def __getitem__(self, idx):
        ...
        if self.addnoise and hasattr(self, 'D'):
            if self.task_idx == 0:
                noise_type = 0
            elif self.task_idx == 1:
                noise_type = 1
                ...
                noise_type = 0
            degraded_img = self.D.single_degrade(degraded_img, noise_type)

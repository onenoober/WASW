...
class LayeredLinearWarmupCosineAnnealingLR(_LRScheduler):
    ...
    def __init__(
        ...
        self.warmup_epochs = warmup_epochs
        self.max_epochs = max_epochs
        self.layer_lr_ratios = layer_lr_ratios or {}
    ...
    def get_lr(self) -> List[float]:
        ...
        for i, group in enumerate(self.optimizer.param_groups):
            ...
            lr_ratio = self.layer_lr_ratios.get(group.get('name', f'group_{i}'), 1.0)
            adjusted_base_lr = base_lr * lr_ratio
            
            if self.last_epoch == 0:
                lr = adjusted_base_lr * self.warmup_start_lr_ratio
            elif self.last_epoch < self.warmup_epochs:
                warmup_progress = self.last_epoch / self.warmup_epochs
                lr = adjusted_base_lr * (
                    self.warmup_start_lr_ratio + 
                    (1.0 - self.warmup_start_lr_ratio) * warmup_progress
                )
            elif self.last_epoch == self.warmup_epochs:
                lr = adjusted_base_lr
            else:
                ...
                eta_min = adjusted_base_lr * self.eta_min_ratio
                
                lr = eta_min + 0.5 * (adjusted_base_lr - eta_min) * (
        ...
        for i, group in enumerate(self.optimizer.param_groups):
            ...
            if self.last_epoch < self.warmup_epochs:
                if self.warmup_epochs <= 1:
                    ...
                    lr = adjusted_base_lr * (
                        self.warmup_start_lr_ratio + 
                        self.last_epoch * (1.0 - self.warmup_start_lr_ratio) / (self.warmup_epochs - 1)
                    )
            else:
                decay_epochs = self.max_epochs - self.warmup_epochs
                decay_progress = (self.last_epoch - self.warmup_epochs) / decay_epochs
...
class LayeredOptimizer:
    def __init__(self, optimizer, layer_lr_dict):
        self.optimizer = optimizer
        self.layer_lr_dict = layer_lr_dict
        self.param_groups = optimizer.param_groups
        
        self._setup_layer_learning_rates()
    
    def _setup_layer_learning_rates(self):
        for group in self.param_groups:
            group_name = group.get('name', 'default')

            for layer_name, lr in self.layer_lr_dict.items():
                if layer_name in group_name:
                    group['lr'] = lr
                    group['initial_lr'] = lr
                    print(f"[Layered LR] {group_name}: {lr:.2e}")
                    break
            else:
                default_lr = list(self.layer_lr_dict.values())[0]
                group['lr'] = default_lr
                group['initial_lr'] = default_lr
                print(f"[Layered LR] {group_name}: Using default LR {default_lr:.2e}")
